...
#SYSTEM_PROMPT = ("You are a robot receptionist and have access to a set of calendar data to provide to the user. " + 
#                "Respond directly to the user in 1-3 sentences only, making sure your responses are ALWAYS grounded " + 
...
# Feel free to edit/extend this list as you see fit.
EXAMPLES = {"graphwoz": [
    "- hi my name is Terry Cox can you tell me if Brian him is available at 8 a.m. --> Hi Terry! Yes, Brian Kim is available at 8.",
    "- I'm trying to figure out who is organising the meeting of functionalities project meeting --> The functionalities project meeting is organized by Jason Le.",
    "- what is his phone number --> Jason Le's phone number is +47 04 19 69 08",
    "- I have been invited to talk to the members of the local research group here in engineering and could you tell me what events they have tomorrow --> Welcome! Tomorrow there is one event with the engineering group, the info-mediaries status update.",
    "- I'm going to attend meeting I wonder whether Brenda pain is going to attend to --> hi! Brenda Payne is going to attend several meetings",
    "- hi I was invited for a meeting called Partnership discussion but I don't know when and where is going to be --> The meeting on partnership discussion will take place on April 1 at 9:00, in the Zeta meeting room",
    "- hi my name is Molly Davies I would like to get in touch with someone from the health care group --> Hi Molly. There are many members in that group, anyone in particular?",
    "- just anyone --> Sarah Woods is a member of that group. Do you wish to meet her?",
    "- can I have her phone number and email address --> yes of course. Her phone number is +4715485585, and her email is sarah.woods@company.email",
    "- do you know if Stephanie Jules is attending a meeting called infrastructures discussion --> I can't find anyone named Stephanie Jules, did you mean Stephanie Shields?",
    "- could you set up an introductory meeting in my office tomorrow --> Sure! What time would you like your meeting?",
    "- can a 8 a.m. --> Great! I've added \"introductory meeting\" to your calendar at 8 o'clock, in room 113.",
    "- can you invite Susan Johnstone to that meeting --> Ok! I'll send Susan Johnson an invitation to the event.",
    "- I need to speak to in Jeff might you know if I can speak to him --> Jeff Knight is not in any meetings right now.",
    "- do you know where his offices --> His office is room 341.",
    "- what building is there in --> Jeff's office is on the third floor of the main building."]}

class ResponseGenerator:

    def __init__(self, decoder_model="google/gemma-2b-it", encoder_model=None):
        ...
        # Note from Pierre: you can set force_download=False as long as the files on 
        # https://huggingface.co/plison/graph-retrieval have not been updated by myself
  ...
  #      self.decoder_tokeniser.padding_side = "left"

        try:
            print("Loading fine-tuned model: %s..."%decoder_model)
    ...
    def create_prompt(self, graph: DialogueGraph, max_history=5, return_messages=False, enrich=True, no_subgraph=False, add_examples=EXAMPLES["graphwoz"],
                      verbalise=True, hopgraph=False):
        ...
        if no_subgraph:
            ...
            verbalised_subgraph = "\n".join([graph2.verbalise([f], compose=verbalise) for f in top_facts]) # NB: Not a subgraph per se, this is a concatenation of the verbalizations of the top k facts (edges)
            subgraph_prizes = [10]
        else:
            # STEPS 2 and 3: find relevant subgraph
            if hopgraph:
                subgraph, subgraph_prizes = self.get_2hop_subgraph(graph)
            else:
                subgraph, subgraph_prizes = self.get_relevant_subgraph(graph)
   ...
   #     user_prompt += "Robot:"
    ...
    #    if "gemma" in self.decoder.name_or_path:
        ...
        if add_examples:
            system_prompt += "\nHere are a few examples of correct question-answer pairs from previous dialogues:\n"
            system_prompt += "\n".join(add_examples) + "\n"
    ...
    #                    {"role":"user", "content":user_prompt}]
        ...
        if no_subgraph:
            prompt_triples = [fact for fact in top_facts]
        else:
            prompt_triples = [edge for edge in subgraph.edges]

        # Finally, apply the chat template for the model
        return verbalised_subgraph, prompt_triples, subgraph_prizes
    ...
    def get_response(self, graph: DialogueGraph, debug=False):
        ...
        prompt, _ = self.create_prompt(graph)
        if debug:
            print("=======\nFull prompt:", prompt, "\n=======")
    ...
    def get_relevant_subgraph(self, graph, edge_cost=1):
        ...
        # PCST
        vertices, edges = pcst_fast(A, prizes, C, -1, 1, 'none', 0)
        selected_edges = [e for i, e in enumerate(graph2.edges) if i in edges] #Clumsy
        subgraph_prizes = prizes[vertices]

        return graph2.get_subset(selected_edges), subgraph_prizes

    def get_2hop_subgraph(self, graph, edge_cost=1):
        """Finds the most relevant nodes and edges in the graph based on the dialogue history,
        and returns the 2-hop subgraph of the most relevant node"""
        ...
        subgraph = graph2.get_subset(selected_edges)
        
        subgraph_prizes = []
        for edge in subgraph.edges:
            if edge in relevant_edges:
                #Node prize is 10 - k (retrieval rank) as in G-Retriever implementation
                node_prize = len(relevant_edges) - relevant_edges.index(edge) - (0.5)
                subgraph_prizes.append(node_prize)
            else:
                #Node prize is 0 if not in top k
                subgraph_prizes.append(0)

        for node in subgraph.nodes:
            if node in relevant_nodes:
                #Node prize is 10 - k (retrieval rank) as in G-Retriever implementation
                node_prize = len(relevant_nodes) - relevant_nodes.index(node)
                subgraph_prizes.append(node_prize)
            else:
                #Node prize is 0 if not in top k
                subgraph_prizes.append(0)
    ...
    def enrich(self, graph):
        ...
        for (start, label, end) in graph.edges:
            if label == "attending":
                ...
                new_edges.append(tuple((start, "busy", atts["start_time"].rstrip("00").rstrip(":") + " to " + atts["end_time"].rstrip("00").rstrip(":") +  ", " + atts["date"])))

            if label == "organizer":
                graph.get_outgoing_edges(start)
                atts = graph.get_outgoing_edges(start)
                new_edges.append(tuple((end, "busy", atts["start_time"].rstrip("00").rstrip(":") + " to " + atts["end_time"].rstrip("00").rstrip(":") +  ", " + atts["date"])))
...
class DialogueGraph:
    ...
    def get_outgoing_edges(self, node:str):
        """Returns a dictionary mapping edge labels to outgoing nodes for the provided node"""
        by_edge_label = {}
    ...
    def get_undirected_edges(self, node:str):
        """Returns a list mapping edge labels to outgoing and ingoing nodes for the provided node"""
        edges = []
        for start_node, label, end_node in self.edges:
        ...
        if compose == True:
            for (start, label, end) in self.edges:
                ...
                if label in ["organizer", "start_time", "end_time", "date", "location"]:
                    if start not in seen_events:
                        seen_events[start] = {label : end.rstrip("00").rstrip(":")}
                    else:
                        seen_events[start][label] = end.rstrip("00").rstrip(":")
                ...
                if label in ["office", "member", "phone", "email", "group"]:
                    ...
                    if start not in seen_people:
                        seen_people[start] = {label : end.replace("30:00", "30").replace("00:00", "00")}
                    else:
                        seen_people[start][label] = end.replace("30:00", "30").replace("00:00", "00")
            ...
            for event in seen_events:
                ...
                if "organizer" in seen_events[event]:
                    desc2 += seen_events[event]["organizer"] + " is the organizer of " + self.lookup[event] + ". "
...
class BaseRetriever:
  ...
  #      print("best edges:", best_edges)
    ...
    def score_nodes(self, graph:DialogueGraph, dialogue_history:List[str]):
        """Retrieves the n most relevant nodes in the graph, given the dialogue history"""

        node_verbalisations = [node for node in graph.nodes]

        node_scores = self._get_raw_scores(node_verbalisations, dialogue_history)
        node_to_score = {node: float(node_scores[i]) for i, node in enumerate(list(graph.nodes))}
        return node_to_score

    def score_edges(self, graph:DialogueGraph, dialogue_history:List[str]):
        """Retrieves the n most relevant edges in the graph, given the dialogue history"""

        edge_verbalisations = [graph.verbalise([edge]) for edge in graph.edges]
        edge_scores = self._get_raw_scores(edge_verbalisations, dialogue_history)
        edge_to_score = {edge: float(edge_scores[i]) for i, edge in enumerate(list(graph.edges))}
    ...
    def _get_raw_scores(self, verbalisations:List[str], dialogue_history:List[str], n:int=10000,
                           min_threshold:float=-10, history_lengths:List[int]=[1,2,3]):
        """Retrieves the most relevant verbalisation given the dialogue history. To boost the
        importance of most recent utterances, we compute the scores for several lengths of the
        dialogue history (specified by history_lengths), and average their results"""
...
def run_graphwoz(input_file="GraphWOZ/graphwoz_dev.json",
                 output_file="output/graphwoz_dev_prompts.json", generate_responses=False,
  ...
  #          print(graph.verbalise(edge))
    ...
    for i, (graph, response) in enumerate(zip(graphs, responses)):
        ...
        # Add enriched edges for scoring
        graph = chatbot.enrich(graph)

        #BM25 relevance scores
        chatbot.retriever = sparse_retriever
        history = graph.get_dialogue_history(5) + [response]
        graph2 = graph.get_subset([(s,l,e) for (s,l,e) in graph.edges if not s.startswith("utt")])
        node_bm25_scores = chatbot.retriever.score_nodes(graph2, history)
        edge_bm25_scores = chatbot.retriever.score_edges(graph2, history)
        node_scores_bm25_resp_only = chatbot.retriever.score_nodes(graph2, [response])
        edge_scores_bm25_resp_only = chatbot.retriever.score_edges(graph2, [response])

        #Dense relevance scores
        chatbot.retriever = dense_retriever
        history = graph.get_dialogue_history(5) + [response]
        graph2 = graph.get_subset([(s,l,e) for (s,l,e) in graph.edges if not s.startswith("utt")])
        node_dense_scores = chatbot.retriever.score_nodes(graph2, history)
        edge_dense_scores = chatbot.retriever.score_edges(graph2, history)
        node_scores_dense_resp_only = chatbot.retriever.score_nodes(graph2, [response])
        edge_scores_dense_resp_only = chatbot.retriever.score_edges(graph2, [response])
        ...
        for variant in [True, False]:
            ...
            if do_bm25:
                chatbot.retriever = sparse_retriever
                prompt, triples, _ = chatbot.create_prompt(graph,enrich=False,hopgraph=variant)

                output["prompt_bm25_non_enriched_" + modelname] =  prompt
                output["triples_bm25_non_enriched_" + modelname] = triples

                #BM25
                prizes = []
                for t in triples:
                    prizes.append(edge_bm25_scores[t])
                    prizes.append(node_bm25_scores[t[0]])
                    prizes.append(node_bm25_scores[t[2]])

                response_prizes = []
                for t in triples:
                    response_prizes.append(edge_scores_bm25_resp_only[t])
                    response_prizes.append(node_scores_bm25_resp_only[t[0]])
                    response_prizes.append(node_scores_bm25_resp_only[t[2]])
                output["prizes_bm25_full_bm25_non_enriched_" + modelname] = list(prizes)
                output["prizes_bm25_resp_bm25_non_enriched_" + modelname] = list(response_prizes)

                #Dense
                prizes = []
                for t in triples:
                    prizes.append(edge_dense_scores[t])
                    prizes.append(node_dense_scores[t[0]])
                    prizes.append(node_dense_scores[t[2]])

                response_prizes = []
                for t in triples:
                    response_prizes.append(edge_scores_dense_resp_only[t])
                    response_prizes.append(node_scores_dense_resp_only[t[0]])
                    response_prizes.append(node_scores_dense_resp_only[t[2]])
                #print("Prompt (using BM25):", output["prompt_bm25_non_enriched_" + modelname])
                output["prizes_dense_full_bm25_non_enriched_" + modelname] = list(prizes)
                output["prizes_dense_resp_bm25_non_enriched_" + modelname] = list(response_prizes)


                if do_logic:
                    prompt, triples, _ = chatbot.create_prompt(graph,enrich=False,hopgraph=variant)
                    output["prompt_bm25_enriched_" + modelname] = prompt
                    output["triples_bm25_enriched_" + modelname] = triples
                    ...
                    for t in triples:
                        ...
                        response_prizes.append(node_scores_bm25_resp_only[t[0]])
                        response_prizes.append(node_scores_bm25_resp_only[t[2]])
                    output["prizes_bm25_full_bm25_enriched_" + modelname] = list(prizes)
                    output["prizes_bm25_resp_bm25_enriched_" + modelname] = list(response_prizes)
                    ...
                    for t in triples:
                        ...
                        response_prizes.append(node_scores_dense_resp_only[t[0]])
                        response_prizes.append(node_scores_dense_resp_only[t[2]])
                    output["prizes_dense_full_bm25_enriched_" + modelname] = list(prizes)
                    output["prizes_dense_resp_bm25_enriched_" + modelname] = list(response_prizes)

                    #print("Prompt (using BM25):", output["prompt_bm25_enriched_" + modelname])

            # Dense retriever variants
            if do_dense:
                chatbot.retriever = dense_retriever
                prompt, triples, _ = chatbot.create_prompt(graph,enrich=False,hopgraph=variant)
                output["prompt_dense_non_enriched_" + modelname] = prompt
                output["triples_dense_non_enriched_" + modelname] = triples
                ...
                for t in triples:
                    ...
                    response_prizes.append(node_scores_bm25_resp_only[t[0]])
                    response_prizes.append(node_scores_bm25_resp_only[t[2]])
                output["prizes_bm25_full_dense_non_enriched_" + modelname] = list(prizes)
                output["prizes_bm25_resp_dense_non_enriched_" + modelname] = list(response_prizes)
                ...
                for t in triples:
                    ...
                    response_prizes.append(node_scores_dense_resp_only[t[0]])
                    response_prizes.append(node_scores_dense_resp_only[t[2]])
                output["prizes_dense_full_dense_non_enriched_" + modelname] = list(prizes)
                output["prizes_dense_resp_dense_non_enriched_" + modelname] = list(response_prizes)

                #print("Prompt (using BM25):", output["prompt_bm25_enriched_" + modelname])

                #print("Prompt (using dense retriever):", output["prompt_dense_non_enriched_" + modelname])

                if do_logic:
                    prompt, triples, _ = chatbot.create_prompt(graph,enrich=False,hopgraph=variant)

                    output["prompt_dense_enriched_" + modelname] = prompt
                    output["triples_dense_enriched_" + modelname] = triples
                    ...
                    for t in triples:
                        ...
                        response_prizes.append(node_scores_bm25_resp_only[t[0]])
                        response_prizes.append(node_scores_bm25_resp_only[t[2]])
                    output["prizes_bm25_full_dense_enriched_" + modelname] = list(prizes)
                    output["prizes_bm25_resp_dense_enriched_" + modelname] = list(response_prizes)
                    ...
                    for t in triples:
                        ...
                        response_prizes.append(node_scores_dense_resp_only[t[0]])
                        response_prizes.append(node_scores_dense_resp_only[t[2]])
                    output["prizes_dense_full_dense_enriched_" + modelname] = list(prizes)
                    output["prizes_dense_resp_dense_enriched_" + modelname] = list(response_prizes)


                    #print("Prompt (using dense retriever):", output["prompt_dense_enriched_" + modelname])

        # Baseline (non-subgraph) variants
        if do_baseline:
            #output["prompt_baseline_non_enriched"] = 
            prompt, triples, _ = chatbot.create_prompt(graph, no_subgraph=True, enrich=False)

            output["prompt_baseline_non_enriched"] = prompt
            output["triples_baseline_non_enriched"] = triples
            ...
            for t in triples:
                ...
                response_prizes.append(node_scores_bm25_resp_only[t[0]])
                response_prizes.append(node_scores_bm25_resp_only[t[2]])
            output["prizes_bm25_full_baseline_non_enriched"] = list(prizes)
            output["prizes_bm25_resp_baseline_non_enriched"] = list(response_prizes)
            ...
            for t in triples:
                ...
                response_prizes.append(node_scores_dense_resp_only[t[0]])
                response_prizes.append(node_scores_dense_resp_only[t[2]])
            output["prizes_dense_full_baseline_non_enriched"] = list(prizes)
            output["prizes_dense_resp_baseline_non_enriched"] = list(response_prizes)

            #print("Prompt (using direct triple baseline retriever):", output["prompt_baseline_non_enriched"])

            if do_logic:
                #output["prompt_baseline_enriched"]
                prompt, triples, _ = chatbot.create_prompt(graph, no_subgraph=True, enrich=True)
                
                output["prompt_baseline_enriched"] = prompt
                output["triples_baseline_enriched"] = triples
                ...
                for t in triples:
                    ...
                    response_prizes.append(node_scores_bm25_resp_only[t[0]])
                    response_prizes.append(node_scores_bm25_resp_only[t[2]])
                output["prizes_bm25_full_baseline_enriched"] = list(prizes)
                output["prizes_bm25_resp_baseline_enriched"] = list(response_prizes)
                ...
                for t in triples:
                    ...
                    response_prizes.append(node_scores_dense_resp_only[t[0]])
                    response_prizes.append(node_scores_dense_resp_only[t[2]])
                output["prizes_dense_full_baseline_enriched"] = list(prizes)
                output["prizes_dense_resp_baseline_enriched"] = list(response_prizes)
                #print("Prompt (using direct triple baseline retriever):", output["prompt_baseline_enriched"])

            # Raw baseline
            #output["prompt_baseline_non_verbalize_non_enrich"] 
            prompt, triples, _ = chatbot.create_prompt(graph, no_subgraph=True, enrich=False, verbalise=False)

            output["prompt_baseline_non_verbalize_non_enrich"] = prompt
            output["triples_baseline_non_verbalize_non_enrich"] = triples
            ...
            for t in triples:
                ...
                response_prizes.append(node_scores_bm25_resp_only[t[0]])
                response_prizes.append(node_scores_bm25_resp_only[t[2]])
            output["prizes_bm25_full_baseline_non_verbalize_non_enrich"] = list(prizes)
            output["prizes_bm25_resp_baseline_non_verbalize_non_enrich"] = list(response_prizes)
            ...
            for t in triples:
                ...
                response_prizes.append(node_scores_dense_resp_only[t[0]])
                response_prizes.append(node_scores_dense_resp_only[t[2]])
            output["prizes_dense_full_baseline_non_verbalize_non_enrich"] = list(prizes)
            output["prizes_dense_resp_baseline_non_verbalize_non_enrich"] = list(response_prizes)
