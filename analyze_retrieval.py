...
class ResponseGenerator:
    ...
    def create_prompt(self, graph: DialogueGraph, max_history=5, return_messages=False, enrich=True, verbalise=True, no_subgraph=False):
        ...
        if no_subgraph:
            ...
            top_facts = self.retriever.get_most_relevant_edges(graph, history)
            ...
            #subgraph, _ = self.get_relevant_subgraph(graph)
            subgraph, _ = self.get_2hop_subgraph(graph)
    ...
    def convert_graph_pcst(self, graph, relevant_nodes, edge_cost=1):
        ...
        C = np.full(shape=(len(A)), fill_value=edge_cost, dtype=np.float64)
    ...
    def get_relevant_subgraph(self, graph, edge_cost=2):
        ...
        A, C, prizes, indexed_edges = self.convert_graph_pcst(graph2,relevant_nodes,edge_cost=edge_cost)
        # PCST
...
def run_graphwoz(input_file="data/graphwoz_dev.json",
    ...
    #k_graphs, k_responses = loadKVRET("data/kvret_dataset_public/kvret_train_public.json", return_responses=True)
  ...
  #          print(graph.verbalise(edge))
    ...
    for i, (graph, response) in enumerate(zip(graphs, responses)):
        print("Turn %i:"%(i+1))
        subgraph, sub_prize = chatbot.get_relevant_subgraph(graph,edge_cost=2.5)
        bm25_edges = [edge for edge in subgraph.edges]
        bm25_nodes = [start for (start, label, end) in bm25_edges] + [end for (start, label, end) in bm25_edges]

        RG = nx.Graph()
        for (start, label, end) in bm25_edges:
            RG.add_edge(start, end)

        subgraph_prizes.append(np.sum(sub_prize))

        # Baseline triple retrieval
        history = graph.get_dialogue_history(5)
        baseline_edges = chatbot.retriever.get_most_relevant_edges(graph, history)
        baseline_nodes = [start for (start, label, end) in baseline_edges] + [end for (start, label, end) in baseline_edges]

        subgraph_node_sizes.append(len(set(bm25_nodes)))
        subgraph_edge_sizes.append(len(bm25_edges))

        print("\nSubgraph:", subgraph.verbalise(compose=True))
        print("\nSubgraph Prize:", np.sum(sub_prize))
        print("\nBaseline:", "\n".join([graph.verbalise([f], compose=True) for f in baseline_edges]))
        print("\n")
        ...
        BG = nx.Graph()
        for (start, label, end) in baseline_edges:
            BG.add_edge(start, end)

        ismags = nx.isomorphism.ISMAGS(RG,BG)
        #print(list(ismags.largest_common_subgraph()))

        print("Percent of baseline edges in subgraph: ", len([e for e in baseline_edges if e in bm25_edges]) / 5)
        print("Percent of baseline nodes in subgraph: ", len([e for e in baseline_nodes if e in bm25_nodes]) / len(baseline_nodes))
        print("Percent of subgraph edges in baseline: ", len([e for e in bm25_edges if e in baseline_edges]) / len(bm25_edges))
        print("Percent of subgraph nodes in baseline: ", len([e for e in bm25_nodes if e in baseline_nodes]) / len(bm25_nodes))
        print("\n")

        edge_fractions.append(len([e for e in baseline_edges if e in bm25_edges]) / 5)
        node_fractions.append(len([e for e in baseline_nodes if e in bm25_nodes]) / len(baseline_nodes))

    print("Average fraction of baseline edges in subgraph:", np.average(edge_fractions))
    print("Average fraction of baseline nodes in subgraph:", np.average(node_fractions))
    print("Average subgraph edge size:", np.average(subgraph_edge_sizes))
    print("Average subgraph node size:", np.average(subgraph_node_sizes))
    print("Average subgraph prize:", np.average(subgraph_prizes))
